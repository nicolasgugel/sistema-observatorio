...
class GroverScraper(CompetitorBase):
    ...
    def _parse_item(self, item: dict, source_url: str) -> Optional[PriceRow]:
        ...
        # Nueva estructura: cheapestRentalPlan está en cheapestVariants[0]
        cheapest_variants = item.get("cheapestVariants") or []
        cheapest = (
            cheapest_variants[0].get("cheapestRentalPlan")
            if cheapest_variants
            else item.get("cheapestRentalPlan")
        ) or {}
        if cheapest:
