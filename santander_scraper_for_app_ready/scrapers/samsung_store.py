...
class SamsungStoreScraper(CompetitorBase):
    ...
    async def scrape(self) -> list[PriceRow]:
        ...
        rows = await self._revalidate_cash_rows(fetcher, rows)
    ...
    async def _revalidate_cash_rows(self, fetcher, rows: list[PriceRow]) -> list[PriceRow]:
        """
        ...
        async def validate(row: PriceRow) -> Optional[PriceRow]:
            async with sem:
                try:
                    return await self._revalidate_cash_row(fetcher, row)
                except Exception as e:
                    logger.debug(f"[Samsung Store] PDP validate error {row.source_url}: {e}")
                    return row

        validated = await asyncio.gather(*[validate(row) for row in rows])
        kept = [row for row in validated if row is not None]
        dropped = len(validated) - len(kept)
        if dropped:
            logger.info(f"[Samsung Store] Validacion PDP: descartadas {dropped} filas incompatibles")
        return kept

    async def _revalidate_cash_row(self, fetcher, row: PriceRow) -> Optional[PriceRow]:
        ...
        response = await fetcher.async_fetch(row.source_url)
